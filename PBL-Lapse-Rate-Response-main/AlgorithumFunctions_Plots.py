...
def pblRI(startData,data):
    ...
    plt.plot(ri,hi,linewidth=.75)
    plt.xlim(-1,1); plt.ylim(0,3000)
    plt.plot([0.25]*2,plt.ylim())
    plt.xlabel("$Ri_{B}$",size=10)
    plt.ylabel("Altitude AGL (m)",size=10)
    plt.axhline(round(np.interp(0.25,ri,hi)),plt.xlim()[0],plt.xlim()[1],linestyle='dashed',color='grey')
    ax.yaxis.set_minor_locator(MultipleLocator(250))
    ax.xaxis.set_minor_locator(MultipleLocator(0.25))
    plt.tight_layout()
    plt.savefig("C:/Users/alex/OneDrive - University of Idaho/ISGC/Publication/Figures/Manuscript Figures/RI_M.tiff",dpi=300)
    plt.show()
...
def pblPT(startData,noct):
    ...
    # fig,ax = plt.subplots(figsize=(3.25,2.15))
    # plt.plot(dp,heights,linewidth=.75)
    # plt.ylim(0,3000)
    # plt.xlabel("PT Gradient (∇θ)",size=10)
    # plt.ylabel("Altitude AGL (m)", size=10)
    # plt.axhline(arr,-1,1,linestyle='dashed',color='grey')
    # ax.yaxis.set_minor_locator(MultipleLocator(250))
    # ax.xaxis.set_minor_locator(MultipleLocator(0.01))
    # plt.tight_layout() 
    # # plt.savefig("C:/Users/alex/OneDrive - University of Idaho/ISGC/Publication/Figures/Manuscript Figures/PT_M.tiff",dpi=300)
    # plt.show()
...
def pblVPT(startData,saveName):
    ...
    # fig,ax = plt.subplots(figsize=(3.25,2.15))
    # plt.plot(vpt,hi,linewidth=.75)
    # plt.plot(vert_ln, plt.ylim()) 
    # plt.hlines(root_max,negXlim,posXlim,colors='grey',linestyles='dashed')     #Plots a vertical line at the X-value of VPT[1]
    # axes = plt.gca() 
    # axes.set_xlim([negXlim,posXlim])                                           #Centers X-bounds about Vertical line, easy to view
    # axes.set_ylim([0,3000])  
    # plt.yticks(size=9)                  #to plot dashed line value, ([0,1000,root_max,2000,3000])   
    # plt.xticks(size=9)  
    # ax.yaxis.set_minor_locator(MultipleLocator(250))                                             #Average range to see values
    # ax.xaxis.set_minor_locator(MultipleLocator(5))                                             #Average range to see values
    # plt.xlabel("VPT [°K]",size=10)
    # plt.ylabel("Altitude AGL (m)",size=10)
    # # plt.title('VPT PBL Determination \n %s' %(saveName),fontsize=12)
    # plt.tight_layout()
    # # plt.savefig("C:/Users/alex/OneDrive - University of Idaho/ISGC/Publication/Figures/Manuscript Figures/VPT_M.tiff",dpi=300)
    # plt.show()  
    return root_max
...
def fullOutput(RunProgram,output1,output2,saveName,PBLResults):
    ...
    output2.append((saveName,RunProgram[0],RunProgram[1],RunProgram[2],PBLResults[1],PBLResults[0]))
...
def printConsole(TotalResults,savePath):
    ...
    ########################################################################
    header2 = ['Launch Title','RI','VPT','PT','Used Method','PBL [m]']
    ...
    for i in range(len(results2)):
        if i == 0:
            print("\n\nPBL Method Results")
            print(dash2)
            print('{:<20s}{:>14s}{:>16s}{:>13s}{:>24s}{:>15s}'.\
                  format(results2[i][0],results2[i][1],results2[i][2],results2[i][3],results2[i][4],results2[i][5]))
            print(dash2)
        else:
            print('{:<20s}{:>3}{:>7s}{:>15s}{:>14s}{:>7s}{:>12s}{:>17s}'.\
                  format(results2[i][0],'|',results2[i][1],results2[i][2],results2[i][3],'|',results2[i][4],results2[i][5]))
...
def saveTxt(TotalResults,savePath,fileNames):
    ...
    with open('%s/%s.txt'%(savePath,fileNames[0]),'w',encoding='utf-8')as f:
        for i in range(len(results2)):
            if i == 0:
                print("\nProgram Operated On: "+time.strftime("%a, %d %b %Y %H:%M:%S"),file=f)
                print(dash2,file=f)
                print('{:<20s}{:>14s}{:>16s}{:>13s}{:>24s}{:>15s}'.\
                      format(results2[i][0],results2[i][1],results2[i][2],results2[i][3],results2[i][4],results2[i][5]),file=f)
                print(dash2,file=f)
            else:
                print('{:<20s}{:>3}{:>7s}{:>15s}{:>14s}{:>7s}{:>12s}{:>17s}'.\
                      format(results2[i][0],'|',results2[i][1],results2[i][2],results2[i][3],'|',results2[i][4],results2[i][5]),file=f)
...
def mills700(data,millsOutput,saveName):#######################################
      mills = data['Alt'].loc[abs(data['P']-900).idxmin()]
